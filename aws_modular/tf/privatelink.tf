...
data "aws_iam_policy_document" "s3_vpc_endpoint_policy" {
  ...
  statement {
    ...
    condition {
      ...
      values   = var.enable_endpoint_policy_hardening ? (var.network_configuration != "custom" ? [aws_vpc_endpoint.backend_rest[0].id] : var.allowed_source_vpce_ids) : []
    }
  }
  ...
  statement {
    ...
    condition {
      ...
      values   = var.enable_endpoint_policy_hardening ? var.allowed_serverless_org_paths : []
    }
  }
...
resource "aws_vpc_endpoint" "backend_rest" {
  ...
  vpc_id              = module.vpc[0].vpc_id
  service_name        = local.workspace[var.region]
...
resource "aws_vpc_endpoint" "backend_relay" {
  ...
  vpc_id              = module.vpc[0].vpc_id
  service_name        = local.scc_relay[var.region]
  vpc_endpoint_type   = "Interface"
  security_group_ids  = [aws_security_group.privatelink[0].id]
  subnet_ids          = module.vpc[0].intra_subnets
  private_dns_enabled = true
