...
# Optional grants: executor user
resource "databricks_grant" "workspace_catalog_executor_user" {
  count = var.executor_user_email != "" ? 1 : 0
  catalog = databricks_catalog.workspace_catalog.name
  principal  = var.executor_user_email
  privileges = ["ALL_PRIVILEGES"]
}

# Optional grants: admin SP
resource "databricks_grant" "workspace_catalog_admin_sp" {
  count = var.admin_sp_client_id != null ? 1 : 0
  catalog = databricks_catalog.workspace_catalog.name
  principal  = var.admin_sp_client_id
  privileges = ["ALL_PRIVILEGES"]
}

# Optional grants: executor SP
resource "databricks_grant" "workspace_catalog_executor_sp" {
  count = var.terraform_sp_client_id != null ? 1 : 0
  catalog = databricks_catalog.workspace_catalog.name
